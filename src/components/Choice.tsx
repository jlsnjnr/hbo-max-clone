...
export function Choice({ onFeedbackTypeChanged }: FeedbackTypeStepProps) {
  return (
    <div className="body-init font-gilroy-medium flex h-screen justify-between items-center flex-col">
      <header>
        <img
          src={splashImg}
          className="align-image w-40 h-40 flex align-center justify-center"
          alt=""
        />
        <h2 className="text-white text-3xl md:text-6xl">Who Is Watching?</h2>
      </header>
      <div className="flex w-[300px] md:w-[411px] 2xl:w-[511px] items-center justify-around ">
        {Object.entries(feedbackTypes).map(([key, value]) => {
          return (
            ...
            >
              <img
                className="w-32 md:w-full 2xl:w-60"
                src={value.image.source}
                alt={value.image.alt}
              />
              <span className="text-white text-xl mt-6 block">
                {value.title}
              </span>
            </button>
          );
      ...
      <div className="flex my-4 w-[300px] md:w-[411px] items-center justify-around">
